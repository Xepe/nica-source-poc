...
resource "google_compute_instance" "default" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      #"(crontab -l ; echo '0 0/4 * * * /usr/bin/python3 /home/gcp_user/main-df.py --runner DataflowRunner --job_name vm-trigger-pipeline-${lower(lookup(var.regions[count.index], "name"))} --service_account_email ${google_service_account.service-project-service-account-data-pipeline.email} --project ${var.data_project} --temp_location gs://${google_storage_bucket.datalake-bucket.*.name[count.index]}/tmp --staging_location gs://${google_storage_bucket.code-bucket.name}/binaries_${lookup(var.regions[count.index], "name")} --subnetwork https://www.googleapis.com/compute/v1/projects/${var.main_project}/regions/${lookup(var.regions[count.index], "region")}/subnetworks/${var.main_project_sub_network}-${lookup(var.regions[count.index], "name")} --region ${lookup(var.regions[count.index], "region")} --requirements_file /home/gcp_user/requirements.txt --db_host ${data.vault_generic_secret.db_creds.data["ip_${lookup(var.regions[count.index], "name")}"]} --db_port ${var.db_port} --db_user ${var.db_user} --db_password \"${data.vault_generic_secret.db_creds.data["password_${lookup(var.regions[count.index], "name")}"]}\" --dest_dataset ${var.dest_dataset}-${lookup(var.regions[count.index], "name")} --dest_bucket ${google_storage_bucket.datalake-bucket.*.name[count.index]} --etl_region ${lookup(var.regions[count.index], "etl_region")}') | sort - | uniq - | crontab -",
