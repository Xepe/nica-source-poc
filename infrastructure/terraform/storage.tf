...
resource "google_storage_bucket" "datalake-bucket" {
    name    = "${var.processing_data_project}-datalake"
...
resource "google_storage_bucket" "etl-tfstate-bucket" {
    name    = "${var.processing_data_project}-etl-tfstate"
    project = "${var.processing_data_project}"
    force_destroy = true
}
...
resource "google_storage_bucket" "code-bucket" {
    name    = "${var.processing_data_project}-code"
    project = "${var.processing_data_project}"
    force_destroy = true    

    provisioner "local-exec" {
      command = "python ./../../code/pipeline_df/main-df.py --runner DataflowRunner --project ${var.processing_data_project} --temp_location gs://${google_storage_bucket.datalake-bucket.name}/tmp --staging_location gs://${google_storage_bucket.code-bucket.name}/binaries --subnetwork https://www.googleapis.com/compute/v1/projects/${var.data_project}/regions/${var.network_region}/subnetworks/${var.data_project_sub_network} --region ${var.network_region} --requirements_file ./../../code/pipeline_df/requirements.txt --template_location gs://${google_storage_bucket.code-bucket.name}/templates/pipeline-template --db_host ${var.db_host} --db_port ${var.db_port} --db_user ${var.db_user} --db_password ${var.db_password} --dest_dataset ${var.dest_dataset} --dest_bucket ${google_storage_bucket.datalake-bucket.name} --etl_region ${var.etl_region}"
    }
}
